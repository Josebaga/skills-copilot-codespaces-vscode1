...
def bitList_base64(bitList, pad):
    ...
    base64codes = []
    for t in SixBitList:     #bucle para recorrer cada una de las listas
        decimal = 0
        for i in range(len(t)): # bucle para recorrer la lista de 6 y transformar
            decimal += t[i] * (2 ** (len(t) - i - 1)) # convertir en decimal
        base64codes.append(decimal)   #añadir a la lista los decimales
    print(f"**INFO LOG** Base 64 Codes: {base64codes}\n")
    b64 = ''.join(b64_dict[c] for c in base64codes) # mapear y eliminar espacios y comas
    if pad != 0:
        b64 += "="*(pad//2)  # añadir un igual por cada 2 ceros cuando se hayan añadido
    return b64
